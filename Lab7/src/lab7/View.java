...
public class View {
	...
	public View(Stage theStage, EventHandler<ActionEvent> startAndStopButtonEvent) {
        theStage.setTitle("Bass");
        
        // add button
        Button startAndStop = new Button("Start/Stop Movement");
        startAndStop.setOnAction(startAndStopButtonEvent);
        ...
        root.getChildren().add(startAndStop);
	...
	public void update(double xLoc, double yLoc, Direction direction,
			BassMode bassMode) {
		x = xLoc;
		y = yLoc;
        ...
        if (modeInd == -1 || currentDirection != lastDirection) {
            ...
            modeInd = rand.nextInt(3);
            lastDirection = currentDirection;
        }

        // TODO fix bassMode to be based on key presses
        this.bassMode = bassMode;
		Image pics = animationSequence[this.bassMode.ordinal()]; // modeInd
